...
public class LayerCulling : MonoBehaviour {
    ...
    static void LayerCullingSetup() {
        ...
        // Tránh add trùng lặp field khi reload
        if (!container.children.Any(c => c.displayName == "Use Layer Culling")) {
    ...
    static void AssignCulling(Camera cameraCullTarget, LayerCulling culling) {
        // HDRP/URP không hỗ trợ layerCullSpherical thông qua script như Built-in
        // Phần này đã được lược bỏ để fix lỗi Render Pipeline.
